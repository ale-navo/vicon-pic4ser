...
{
    // get parameters
    this->declare_parameter<std::string>("hostname", "192.168.1.5");
...
bool Communicator::connect()
{
    ...
    while (!vicon_client.IsConnected().Connected)
    {
        ...
        cout << vicon_client.Connect(hostname).Result << endl;
        
        bool ok = (vicon_client.Connect(hostname).Result == Result::Success); //problema
        cout << ok << endl;
        if (!ok)
        {
            counter++;
